/* 
* Functions to create the main DNAinfo streeteasy
*/

// initialize map
function DNAinfoNYCStreetEasy() {
	...
	if (urlArray[5]) {
		...
		var neighborhood = 'park-slope';      
	} 
	// set zoom and center for this map
	this.center = DNAinfoNYCStreetEasy.center(neighborhood);
    this.zoom = 13;
...
DNAinfoNYCStreetEasy.onEachFeature_ZIPS = function(feature,layer){	
	...
	if (activeLayer == 'sale') {
		...
		if (isNaN(feature.properties.medianaskingprice) || feature.properties.medianaskingprice == -99) {
			var medianaskingprice = "";
		} else {
			var medianaskingprice = "<li>Median sales asking price here is <strong>$" + commaFormat(feature.properties.medianaskingprice) + "</strong>.</li>";
		}

		if (isNaN(feature.properties.medianppsf) || feature.properties.medianppsf == -99) {
			var medianppsf = "";
		} else {
			var medianppsf = "<li>Median sales price is <strong>$" + commaFormat(feature.properties.medianppsf.toFixed(2)) + "</strong> per square foot.</li>";
		}

	    layer.bindPopup("<h5>" + feature.properties.name + "</h5><ul>" + medianaskingprice + medianppsf + "</ul>");
...
DNAinfoNYCStreetEasy.prototype.loadSE = function (){
	var thismap = this;
	...
	function attachDataToTopojson(polygonData) {
		...
		d3.json(NYC_Boundaries, function(data) {
			polyTopojson = topojson.feature(data, data.objects.StreetEasy_NYC_4326).features;
			$.each(polyTopojson, function(i, feature) {
				$.each(polygonData, function(name, value) {
					if (feature.properties.name == name) {
						value.name = name;
						//value.dateperiod = dateFormat.parse(value.dateperiod);
	...
	function drawPolysWData() {
		thismap.ZIPS = L.geoJson(polyTopojson, {
		    style: DNAinfoNYCStreetEasy.getStyleFor_ZIPS,
			onEachFeature: DNAinfoNYCStreetEasy.onEachFeature_ZIPS,
		});
...
DNAinfoNYCStreetEasy.getStyleFor_ZIPS = function (feature){
	if (activeLayer == "sale") {
...
DNAinfoNYCStreetEasy.center = function (neighborhood){
