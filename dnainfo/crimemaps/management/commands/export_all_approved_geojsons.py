...
class Command(BaseCommand):
	...
	def export_geojsons(self):
				...
				for count, NYCdrawing in enumerate(NYCdrawings):
					if NYCdrawing.drawnGeojson:
						...
						with open(os.path.join(__location__, filename), 'wb') as f:
							changed = NYCdrawing.drawnGeojson.replace('\"properties\":{}', '\"properties\":{\"shapeID\":\"'+ str(NYCdrawing.id) +'\", \"neighborhoodLive\":\"'+ str(NYCdrawing.neighborhoodLive.name) +'\", \"otherNeighborhood\":\"' + strip_non_ascii(NYCdrawing.otherNeighborhood) +'\", \"yearsLived\":\"' + str(NYCdrawing.yearsLived) +'\"}')
							f.write(changed)
							f.close()
		...
		for CHIneighborhood in CHIneighborhoods:
			CHIdrawings = neighborhoodDrawCHI.objects.filter(neighborhoodLive=CHIneighborhood, approved=True)
			if len(CHIdrawings) > 0:
				...
				for count, CHIdrawing in enumerate(CHIdrawings):
					if CHIdrawing.drawnGeojson:
						...
						with open(os.path.join(__location__, filename), 'wb') as f:
							changed = CHIdrawing.drawnGeojson.replace('\"properties\":{}', '\"properties\":{\"shapeID\":\"'+ str(CHIdrawing.id) +'\", \"neighborhoodLive\":\"'+ str(CHIdrawing.neighborhoodLive.name) +'\", \"otherNeighborhood\":\"' + strip_non_ascii(CHIdrawing.otherNeighborhood) +'\", \"yearsLived\":\"' + str(CHIdrawing.yearsLived) +'\"}')
							f.write(changed)
							f.close()
