/**
 * map.js: Creates map for selecting which blocks are in the city
 */
...
mapApplication.loadLocationFilter = function () {
	...
	locationFilter.on("change", function (e) {
	    ...
	    for (var i = mapApplication.pointFeatures.length - 1; i >= 0; i--) {
	    	...
	    	if (isInside) {
	    		// if inside, reset everything about this layer, and then simulate a click
	    		// remove "removed" flag
...
mapApplication.loadHood = function () {
	...
	function drawPolys(polyTopojson) {
		mapApplication.HOOD = L.geoJson(polyTopojson, {
		    style: mapApplication.getStyleFor_HOOD,
			onEachFeature: mapApplication.onEachFeature_HOOD
...
mapApplication.onEachFeature_HOOD = function(feature,layer){	
    ...
    layer.on('mouseover', function(ev) {
    	...
    	$(".absolutePostionedDiv").fadeOut("slow");
