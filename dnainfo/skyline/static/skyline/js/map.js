...
mapApplication.initialize = function () {
	...
	//pick carto url based on boro
	mapApplication.pickUrlTableSQL(boro);	
...
mapApplication.loadPluto = function () {
	...
	mapApplication.PLUTO = cartodb.createLayer(mapApplication.map, mapApplication.url, { https: true })
		.addTo(mapApplication.map)
		.on('done', function(layer) {
			var subLayerOptions = {
		      	query: 'SELECT cartodb_id, address, bbl from ' + mapApplication.tableName,
...
mapApplication.featureClick = function (cartodb_id) {
	...
	mapApplication.sql.execute("SELECT * FROM {{tableName}} WHERE cartodb_id= {{cartodb_id}}", { tableName: mapApplication.tableName, cartodb_id: cartodb_id })
		.done(function(geojson) {
			...
			geojson.features[0].properties.color = "#0073a3";
		    geojson.features[0].properties.roofColor = "#0073a3";
...
mapApplication.pickUrlTableSQL = function (boro) {
	// manhattan
	if (boro == 'Manhattan') {
		mapApplication.url = 'https://jd.carto.com/api/v2/viz/250a6fce-5596-11e6-a4e1-0e3ebc282e83/viz.json';
		mapApplication.tableName = 'mnmappluto';
		mapApplication.sql = new cartodb.SQL({ user: 'jd', format: 'geojson' });
	} else if (boro == 'Brooklyn') {
		mapApplication.url = 'https://jdgodchaux.carto.com/api/v2/viz/8ec41b14-55db-11e6-84e3-0ecd1babdde5/viz.json';
		mapApplication.tableName = 'bkmappluto';
		mapApplication.sql = new cartodb.SQL({ user: 'jdgodchaux', format: 'geojson' });
	} else if (boro == 'Queens') {
		mapApplication.url = 'https://jdgodchaux.carto.com/api/v2/viz/f01903a6-55dc-11e6-ac46-0e05a8b3e3d7/viz.json';
		mapApplication.tableName = 'qnmappluto';
		mapApplication.sql = new cartodb.SQL({ user: 'jdgodchaux', format: 'geojson' });		
	} else if (boro == 'Bronx') {
		mapApplication.url = 'https://jd.carto.com/api/v2/viz/607541de-55d6-11e6-883e-0e3ff518bd15/viz.json';
		mapApplication.tableName = 'bxmappluto';
		mapApplication.sql = new cartodb.SQL({ user: 'jd', format: 'geojson' });		
	} else {
		mapApplication.url = 'https://jd.carto.com/api/v2/viz/795aa89a-55d8-11e6-86f0-0e3ebc282e83/viz.json';
...
/* Destroy map */
mapApplication.destroy = function () {
	mapApplication.map.remove();
}
