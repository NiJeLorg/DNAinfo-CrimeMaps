...
from apartment_test_for_danny_nyc.forms import *
...
def intro(request):
	return render(request, 'apartment_test_for_danny_nyc/intro.html', {})
...
def firstMove(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			return HttpResponseRedirect(reverse('whatNeighborhood-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
	...
	return render(request, 'apartment_test_for_danny_nyc/firstMove.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def whatNeighborhood(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('exactLocation-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/whatNeighborhood.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def exactLocation(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('yearMoved-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/exactLocation.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def yearMoved(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('bedrooms-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/yearMoved.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def bedrooms(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('rentSplit-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/bedrooms.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def rentSplit(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('iPaid-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/rentSplit.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def iPaid(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('allPaid-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/iPaid.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def allPaid(request, id=None):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			f = form.save()
			lookupObject = NYCmyFirstApartment.objects.get(pk=f.pk)
			return HttpResponseRedirect(reverse('calc-apartment-test-for-danny-nyc', args=(lookupObject.pk,)))
		else:
			# The supplied form contained errors - just print them to the terminal.
			print form.errors
	else:
	...
	# Bad form (or form details), no form supplied...
	# Render the form with error messages (if any).
	return render(request, 'apartment_test_for_danny_nyc/allPaid.html', {'form':form, 'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject, 'allPaid': allPaid})
...
def calc(request, id=None):
	...
	return render(request, 'apartment_test_for_danny_nyc/calc.html', {'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject})
...
def end(request, id=None):
	...
	url = "https://www.dnainfo.com/new-york/visualizations/dummy-url?results=" + str(id)
	...
	return render(request, 'apartment_test_for_danny_nyc/end.html', {'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject, "bitlyURL": bitlyURL})
...
def results(request, id=None):
	...
	return render(request, 'apartment_test_for_danny_nyc/results.html', {'NYCmyFirstApartmentObject': NYCmyFirstApartmentObject, "bitlyURL": bitlyURL})
...
def rawdataapi(request):
	...
	if request.method == 'GET':
		...
		datas = NYCmyFirstApartment.objects.filter(**kwargs)
		for data in datas:
			if data.whereMoved:
...
def rawdatacsv(request):
	...
	response['Content-Disposition'] = 'attachment; filename="MFA_NYC_Raw_Data.csv"'
...
def summarydataapi(request):
	...
	if request.method == 'GET':
		...
		datas = NYCmyFirstApartment.objects.filter(**kwargs).values('whereMoved_id', 'whenMoved', 'bedrooms').annotate(Avg('allPaid'), Count('id'))
		for data in datas:
			if data['whereMoved_id']:
				hood = neighborhoodNYC.objects.get(id=data['whereMoved_id'])
				hoodname = hood.name
			else:
				hoodname = ''
		...
		datas = NYCmyFirstApartment.objects.filter(**kwargs).values('whenMoved', 'bedrooms').annotate(Avg('allPaid'), Count('id'))
		response['Citywide'] = {}
		for data in datas:
			try:
